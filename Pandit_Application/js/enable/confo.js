...
$("#disconnect_call").on("click", function () {
    ...
    var time = hdnCustomerwalletamount / hdncharges_call_per_minu;
     moneydeducted = time * hdncharges_call_per_minu;
...
function dropcallfun(moneydeducted) {
    debugger
...
function display_c() {
    ...
    var time = hdnCustomerwalletamount / hdncharges_call_per_minu;
     moneydeducted = time * hdncharges_call_per_minu;
    tottime = 60000 * time; // Refresh rate in milli seconds
    finaltime = tottime;
    var refresh = 1000;
    mytime = setTimeout('display_ct(' + refresh + ',' + moneydeducted+')', refresh)
...
function calculatededuction(moneydeducted) {
    debugger
    var time = finaltime - tottime;
    time = time / 60000;
    var bal = time * hdncharges_call_per_minu;
    if (time == 0) {
        bal = moneydeducted
    }
    else {
    ...
    //moneydeducted = moneydeducted - bal;
    $.ajax({
        type: "GET",
        url: '/CallCustomer/Deductedamount/',
        data: { "moneydeducted": bal },
